...
body(1).nFilamentSegments = 40; % number of filament divisions
...
wingSegmentPoints = [0*ones(1,body(1).nWingSegments+1);linspace(-b/2,b/2,body(1).nWingSegments+1);0*ones(1,body(1).nWingSegments+1)];
body(1).FixedStartPoints = wingSegmentPoints(:,1:end-1);
body(1).FixedEndPoints = wingSegmentPoints(:,2:end);
...
for ind1 = 1:body(1).nWingSegments+1
    ...
    body(1).FreeStartPoints = [body(1).FreeStartPoints,...
        kron((0:simulation.dt:(body(1).nFilamentSegments-1)*simulation.dt),simulation.U)+...
        repmat(wingSegmentPoints(:,ind1),[1,body(1).nFilamentSegments])];
    body(1).FreeEndPoints = [body(1).FreeEndPoints,...
        kron((simulation.dt:simulation.dt:body(1).nFilamentSegments*simulation.dt),simulation.U)+...
        repmat(wingSegmentPoints(:,ind1),[1,body(1).nFilamentSegments])];
